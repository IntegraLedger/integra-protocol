...
describe("assemble — the slot guards each refuse for their OWN reason", () => {
  it("mints a minimal record", async () => {
    ...
    expect(env.atrVersion).toBeDefined();
    expect(Object.keys(env)[0]).toBe("atrVersion"); // engine-stamped, always first
    expect(isAtrHash(atrHash)).toBe(true);
  });

  it("refuses a slot claiming the ENGINE-STAMPED atrVersion slot", async () => {
    // `atrVersion` is stamped by the engine, never supplied. Letting a caller's slot set it would let a caller
    // forge the version the record claims conformance to.
    await refuses(
      [...MINIMAL, { slot: "atrVersion", value: "9.9" }],
