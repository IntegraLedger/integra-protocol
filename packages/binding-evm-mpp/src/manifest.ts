 ...
 *
 * The sibling rail proved what a literal costs: `binding-evm-x402` shipped `0.15.1` with `weldGrades` keyed
 * `ERC3009` — the escrow package's COLLECTOR name — while the value an x402 consumer computes from an offer
 * is `eip3009`. The map and the lookup key disagreed inside one published package, so
 * `weldGrades[assetTransferMethod]` answered `undefined`: the grade absent rather than wrong, which reads
 * as a rail declaring no weld grade at all. The profile schema constrains weldGrades VALUES and says
 * nothing about keys, so nothing caught it.
 *
 * ⇒ The key is a constant, not a literal, on every rail. `check:weld-grade-keys` holds it.
 */
export const MPP_COLLECTION_PATH = "authorization";
