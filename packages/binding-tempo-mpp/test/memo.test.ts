...
describe("decodeTip20Memo", () => {
  ...
  it("accepts a BARE 32-byte hex memo — MPP's own parser strips an optional 0x prefix", () => {
    ...
    expect(decodeTip20Memo("ab".repeat(32))).toBe(ATR);
  });

  it("normalizes an upper-case on-chain memo to the lower-case form the stack compares", () => {
    expect(decodeTip20Memo(`0x${"AB".repeat(32)}`)).toBe(ATR);
  });

  it("returns null for a memo that is not 32 bytes (a scan skips it, it is not an error)", () => {
    expect(decodeTip20Memo("0x1234")).toBeNull();
    expect(decodeTip20Memo(`0x${"ab".repeat(33)}`)).toBeNull();
  });

  it("returns null for non-hex content", () => {
    expect(decodeTip20Memo(`0x${"zz".repeat(32)}`)).toBeNull();
  });

  it("returns null for an empty memo — MPP treats the empty string as absent", () => {
    expect(decodeTip20Memo("")).toBeNull();
    expect(decodeTip20Memo("0x")).toBeNull();
  });
  ...
  it("decodes the REAL mainnet memo structurally — shape alone cannot tell attribution from an atrHash", () => {
    ...
    expect(decodeTip20Memo(MAINNET_MEMO)).toBe(MAINNET_MEMO);
  });
});

describe("verifyTip20Memo", () => {
  it("confirms a settlement memo carries exactly the advertised atrHash", () => {
    expect(verifyTip20Memo({ memo: ATR, atrHash: ATR })).toBe(true);
  });

  it("is case-insensitive on both sides", () => {
    expect(
      verifyTip20Memo({ memo: `0x${"AB".repeat(32)}`, atrHash: ATR_UPPER }),
    ).toBe(true);
  });

  it("rejects a different memo", () => {
    expect(
      verifyTip20Memo({ memo: `0x${"cd".repeat(32)}`, atrHash: ATR }),
    ).toBe(false);
  });

  it("rejects a malformed memo", () => {
    expect(verifyTip20Memo({ memo: "0x1234", atrHash: ATR })).toBe(false);
  });

  it("rejects a malformed atrHash rather than comparing garbage", () => {
    expect(verifyTip20Memo({ memo: ATR, atrHash: "nope" })).toBe(false);
  });
});

describe("the guard names the function that refused", () => {
  // `encodeTip20Memo` delegates its shape guard to kernel's `canonicalAtrHash`, passing its own name as the
  ...
  it("encodeTip20Memo's throw identifies encodeTip20Memo", () => {
    expect(() => encodeTip20Memo("0xnope")).toThrow(/encodeTip20Memo/);
  });
});
