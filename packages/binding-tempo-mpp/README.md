...
| `draft-ryan-httpauth-payment-01` (IETF datatracker, 18 Mar 2026) — the IETF mirror of the core scheme, which paymentauth.org publishes as `draft-httpauth-payment-00` and which LCP §C.1 records under both identities | the seven-slot HMAC canonicalization, with `request` (JCS + base64url) as **slot 3** — the table is 0-based — so `methodDetails` is MAC-protected |
