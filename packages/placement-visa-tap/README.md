...
**Drift against LCP §C.6: none material, and one wording gap still open.** LCP §C.6 does not enumerate TAP's
RFC 9421 signature parameters — `created`, `expires`, `keyid`, `alg`, `nonce` and `tag` — and does not say
what `tag` distinguishes; the enumeration above is read off the host, not off the appendix. §C.6 does carry
the one spelling note, "`keyid`, not `kid`, per [RFC 9421]", but attaches it to the *body-object* quartet it
proposes, where TAP itself spells the member `kid` — `keyid` is the message-signature parameter, and this
package follows the host on both. The appendix is an illustration; the host's live specification is what binds.
...
reconciled against LCP §C.6 the same day, and re-read against **LCP §C.6** on 2026-08-12.
