...
describe("a MIXED-container manifest — UCP's actual shape", () => {
  // LCP §C.3, and a shape UCP turned out not to have — kept as a KIT fixture, not as UCP's. The canonical
...
// in the session (agenticcommerce.dev/docs/concepts/extensions, read 2026-07-30; LCP §C.2). The reading
...
//
// LCP §C.2 WITHDREW THAT PATH: "That declaration does not make a new top-level field valid." The
...
// package declares as `tier: "B"`. LCP §C.7's Tier A rested on closed mandates carrying constraints and on
// a `stage` field the live spec does not have; LCP §C.7 retired it — "Tier B — there is no Tier A
