...
describe("encodeAtrMemo / decodeAtrMemo", () => {
  it("hex (canonical): memo data is the 0x-prefixed ASCII hex string", () => {
    const data = encodeAtrMemo(ATR, "hex");
    expect(new TextDecoder().decode(data)).toBe(ATR);
    expect(data.length).toBe(66); // "0x" + 64 hex chars, one byte each
    expect(decodeAtrMemo(data, "hex")).toBe(ATR);
  });

  it("raw: memo data is the 32 raw bytes", () => {
    const data = encodeAtrMemo(ATR, "raw");
    expect(data.length).toBe(32);
    expect(decodeAtrMemo(data, "raw")).toBe(ATR);
  });

  it("lowercases an uppercase atrHash (ATR canon)", () => {
    const data = encodeAtrMemo(ATR.toUpperCase().replace("0X", "0x"), "hex");
    expect(decodeAtrMemo(data, "hex")).toBe(ATR);
  });

  it("fails loud on a malformed atrHash", () => {
    expect(() => encodeAtrMemo("0xdead", "hex")).toThrow(/32-byte/);
    expect(() => encodeAtrMemo("not-hex")).toThrow();
  });

  it("returns null for a memo that is not an atrHash (a scan skips it, not errors)", () => {
    expect(
      decodeAtrMemo(new TextEncoder().encode("hello world"), "hex"),
    ).toBeNull();
    expect(decodeAtrMemo(new Uint8Array([1, 2, 3]), "raw")).toBeNull(); // wrong length
  });
});
...
describe("verifyAtrMemo", () => {
  it("confirms a matching memo and rejects a mismatched one", () => {
    const data = encodeAtrMemo(ATR, "hex");
    expect(verifyAtrMemo({ memoData: data, atrHash: ATR })).toBe(true);
    expect(
      verifyAtrMemo({
        memoData: data,
        atrHash:
  ...
  it("defaults to the hex encoding, and honours an explicit one", () => {
    ...
    expect(
      verifyAtrMemo({ memoData: encodeAtrMemo(ATR, "hex"), atrHash: ATR }),
    ).toBe(true);
    expect(
      verifyAtrMemo({
        memoData: encodeAtrMemo(ATR, "raw"),
        atrHash: ATR,
        encoding: "raw",
      }),
    ).toBe(true);
    expect(
      verifyAtrMemo({ memoData: encodeAtrMemo(ATR, "raw"), atrHash: ATR }),
    ).toBe(false);
  });
  ...
  it("rejects a malformed atrHash argument, even against a well-formed memo", () => {
    const data = encodeAtrMemo(ATR, "hex");
    expect(verifyAtrMemo({ memoData: data, atrHash: "0xdead" })).toBe(false);
    expect(verifyAtrMemo({ memoData: data, atrHash: "" })).toBe(false);
  });

  it("matches case-insensitively, as the ATR canon requires", () => {
    expect(
      verifyAtrMemo({
        memoData: encodeAtrMemo(ATR, "hex"),
