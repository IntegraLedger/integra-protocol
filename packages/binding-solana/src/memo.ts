 ...
 * `decodeAtrMemo` returns null for anything that is not a well-formed atr memo (so a settlement scan can
 * skip non-LCP memos without treating them as errors); `encodeAtrMemo` fails LOUD on a malformed atrHash.
 */
 ...
 *  size, and only safe where every verifier reads the memo as bytes. `decodeAtrMemo` needs to be TOLD
...
export function encodeAtrMemo(
  atrHash: string,
  encoding: MemoEncoding = "hex",
): Uint8Array {
  const canonical = canonicalAtrHash(atrHash, "encodeAtrMemo");
...
export function verifyAtrMemo(inputs: {
  ...
  const decoded = decodeAtrMemo(inputs.memoData, inputs.encoding ?? "hex");
