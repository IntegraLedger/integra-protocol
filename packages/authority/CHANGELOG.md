...
- d4f95bc: The authority package records presented attestations as inspectable envelopes
  ...
  cryptography was checked here. `not-attempted` carries a `depth` naming what could not be read —
  `attestation-not-an-object`, `attestation-profile-not-stated`, `attestation-substrate-not-stated`,
  `attestation-subject-not-stated`, `attestation-ref-not-stated` or `attestation-assurance-unreadable` — so an
  element nobody could interpret is recorded as having been presented rather than dropped. The profile id,
  substrate, subject and `ref` are each a stated gap when absent and when present as an empty string, and an
  `assurance` that is present but unreadable is a stated gap rather than a silently omitted field. The
  substrate is required to be a non-empty string and is compared with nothing, so an attestation on a
  substrate this package has never heard of is recorded rather than refused.
  ...
  `UnwalkedAttestation[]`, each naming why — `addressed-to-the-declared-principal`,
  `addressed-to-no-walked-link` or `attestations-slot-not-keyed` — beside the identifier they were addressed
  to and the same `RecordedAttestation` a link-carried attestation receives. An attestation can never make a
  walk refuse a record.
