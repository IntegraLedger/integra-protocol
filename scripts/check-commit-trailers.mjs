 ...
 *
 * 2. **No agent-authorship trailers.** A `Co-Authored-By:` carrying a vendor identity, or a session URL,
 *    is tooling exhaust, and this repository is world-readable and permanent. The sibling repo bans the
 *    same literals in FILES; commit messages are just as public and were the half nobody scanned.
 *    ⚠️ The markers are matched by VENDOR IDENTITY, never by the name "Claude" — see `FORBIDDEN` for the
 *    human contributor that distinction protects.
 *
...
function subject(argv) {
  // `-n 1 <rev>` rather than `<rev>~1..<rev>` for every tip-only case. A ROOT COMMIT HAS NO PARENT, so the
  // `~1` spelling cannot resolve on a repository's first push — and it fails with "could not resolve",
  // whose message blames a shallow checkout. That is the worst kind of red: correct to fail, wrong about
  // why, on the one push where nobody has prior art to compare against.
  if (argv.length === 2) {
  ...
  if (argv.length === 1)
    return { revs: [argv[0]], how: `explicit range ${argv[0]}` };
  try {
    git("rev-parse", "@{u}");
    return { revs: ["@{u}..HEAD"], how: "unpushed commits (@{u}..HEAD)" };
  } catch {
    return {
      revs: ["-n", "1", "HEAD"],
 ...
 *
 * ⛔⛔ AND THE NAME ALONE IS NOT THE MARKER. **Claude is a common human given name** — French, and borne by
 * contributors this project wants. `co-authored-by:.*claude` rejected `Co-Authored-By: Claude Dubois
 * <claude.dubois@universite.fr>`, a real collaborator, with a message accusing them of being tooling
 * exhaust. A public repository inviting the world cannot ship that. What identifies the machine trailer is
 * the VENDOR IDENTITY — an `@anthropic.com` address, or the name carrying a model designator — never the
 * given name on its own.
 */
const FORBIDDEN = [
  {
    re: /^\s*co-authored-by:[^\n]*<[^>\n]*@anthropic\.com>/im,
    what: "a Co-Authored-By trailer with an Anthropic address",
  },
  {
    re: /^\s*co-authored-by:\s*claude\s+(?:code|opus|sonnet|haiku)\b/im,
    what: "a Co-Authored-By trailer naming a Claude model",
  },
  {
...
const { revs, how } = subject(process.argv.slice(2));
...
try {
  ...
  shas = git("rev-list", "--no-merges", ...revs)
    .split("\n")
    .filter(Boolean);
} catch {
  console.error(
    `check:commit-trailers — could not resolve '${revs.join(" ")}'. The range is the gate's whole subject, so an ` +
...
if (shas.length === 0) {
  console.error(
    `check:commit-trailers — BLIND GATE: '${revs.join(" ")}' resolved to zero commits. A push adds at least one, ` +
