...
Every package here ships at one version — currently **0.19.0** — and the `0.` is a statement rather than an
...
```bash
pnpm install
pnpm verify          # floor-provenance → runner-patch → forbidden-literals → versions → docblocks → declared-imports → hermetic-tests → weld-grade-keys → live-rails → harness-proof → corpus-seal → advisories
