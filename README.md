...
Every package here ships at one version — currently **0.16.1** — and the `0.` is a statement rather than an
...
```bash
pnpm install
pnpm verify          # versions → docblocks → declared-imports → weld-grade-keys → live-rails → harness-proof → corpus-seal → advisories
