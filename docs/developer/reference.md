...
| `placement-acp` | Places an LCP reference into an ACP agentic checkout session's `metadata.legal_context` and reads it back out; the top-level `legal_context` is READ only — LCP §C.2 withdrew the write, and ACP's own schema rejects it. | [README](../../packages/placement-acp/README.md) |
