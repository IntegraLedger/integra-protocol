...
```bash
pnpm verify          # check:floor-provenance → check:runner-patch → check:versions → check:docblocks → check:declared-imports → check:hermetic-tests → check:weld-grade-keys → check:live-rails → check:harness-proof → corpus-seal
